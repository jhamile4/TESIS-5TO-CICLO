...
function Carrito({ carrito, subtotal, onCerrar, onCambiar, onEliminar, onWhatsApp, negocioId }) {
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#E5E7EB]">
            ...
            <button
              onClick={onCerrar}
              className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#F3F4F6] transition-colors cursor-pointer text-[#374151]"
            >
                ...
                <div key={i} className="flex gap-3 bg-[#FAFAFA] rounded-xl p-3 items-center border border-[#E5E7EB]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <button
                        onClick={() => onCambiar(p.pk_id, -1)}
                        className="w-7 h-7 flex items-center justify-center rounded-md bg-white border border-[#E5E7EB] text-[#6B7280] hover:border-[#0D9488] hover:text-[#0D9488] transition-all cursor-pointer"
                      >
                      ...
                      <button
                        onClick={() => onEliminar(p.pk_id)}
                        className="ml-auto w-7 h-7 flex items-center justify-center rounded-md hover:bg-red-50 text-[#9CA3AF] hover:text-red-500 transition-all cursor-pointer"
                      >
      ...
      {/* Modal login comprador */}
      {showLogin && (
        <LoginCompradorModal
          onClose={() => setShowLogin(false)}
          onSuccess={handleLoginExito}
        />
      )}
